...
絞るのは**表示側の仕事**で、次の順に適用する。

1. **`implies` で含意される一般ラベルを隠す。** `石田流` が立てば `三間飛車` `振り飛車` を出さない。
2. 残った**振り先ラベル**（中飛車 / 四間飛車 / 三間飛車 / 向かい飛車 / 右四間飛車 / 袖飛車）が
   複数あれば、**`turn` が最小のものだけ**出す。振り直しの途中経過を並べない。
3. **関係ラベル**（`対抗形` / `相居飛車` / `相振り飛車`）は per-side のタグではなく
   **対局レベル**で出す。ほとんどの対局で立つため、側タグに混ぜると枠が 1 つ潰れる。

```sql
-- 2 の例: 振り先ラベルのうち最初に成立したものを取る
...
- **1 と 2 は衝突しない。** 「振り直しの結果として立つ戦法」は `石田流` のように
  **振り先ラベルを含意する**ので、1 の抑制が意味のある振り直し先を自動的に残す。
  意味のない振り直し（開戦直前に振り直しただけ）は 2 で消える。
