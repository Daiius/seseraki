...
interface Props {
  ...
  /**
   * 表示を開始する手数（既定 0 = 初期局面）。
   * 棋譜詳細は URL の `ply` をここへ渡す（出題・局面検索から「その手」へ飛ぶ導線。
   * prd/05 §2.6）。DEV ギャラリー（`/dev-gallery`）が表示状態を固定するのにも使う。
   * 範囲外の値は端に丸める。
   *
   * ⚠ **`useState` の初期値なので、後から変えても盤は動かない。**
   * 呼び出し側が値を変えて反映させたいときは `key` で作り直す。
   */
