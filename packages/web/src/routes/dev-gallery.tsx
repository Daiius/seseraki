  ...
  return (
    <div className="space-y-8">
      ...
      */}
      ...
      {/*
        🔴 **「評価する」はコントローラー行の 7 つ目・検討していなくても押せる**
        （prd/12 §3.2・決定 2026-09-01）。以前は検討中だけ出す操作パネルのボタンだった。
        `StudyCase` に `session={undefined}` を渡すと `StudyBoard` が
        `createStudySession(baseState)` を作り直す（＝1 段だけの検討していないセッション）ので、
        棋譜再生中と同じ見え方（操作パネルは出ない）でコントローラー行だけを確かめられる。
      */}
      <Case title="コントローラー行・検討していない状態の 7 要素（評価するを含む）">
        {/* アイコン + md 以上はラベル「評価」。他の 6 つは今までどおり */}
        <StudyCase session={undefined} />
      </Case>

      <Case title="コントローラー行・評価中（評価するがスピナーになる）">
        {/*
          🔴 **ボタンの寸法は変えない**（決定・2026-09-01）。アイコン→スピナーの差し替えは
          `size-5` の枠の中だけで起き、md 以上のラベル「評価」は出したまま——行は動かない。
          ⚠ **`studying` を条件から外した評価結果ブロックもここで見える**——検討していなくても
          「評価しています」が盤の下に出て、見出し「この局面の評価」が付く。
        */}
        <StudyCase session={undefined} evalState={{ kind: 'loading' }} />
      </Case>
      ...
      */}
      ...
      {/*
        ...
        `棋譜に戻る`（戻るアイコン）/ `手番`（入れ替えアイコン + ☗☖）/ `成`（字）の 3 つ
        （`評価する`は決定 2026-09-01 でコントローラー行へ移り、ここからは外れた）。
        アイコンのみのボタンは**正方形**で 44px（375px 未満は 40px）を満たし、
        **枠付き**（`btn-outline`）で `aria-label` を持つ。手番の記号は**少し大きめ**にして
        枠内で読めるようにしてある。
      ...
      */}
      ...
      <Case title="検討・操作ボタン 1 行（「成」が無効）">
        {/*
          🔴 **「成」は出し入れせず無効にする**（決定 2026-08-29）。条件付きで消していた頃は
          **成れる手かどうかで行の幅が変わり、他のボタンの位置がズレた**。
          ▲６八金（金は成れない）なので、ここでは「成」が**薄く押せない**状態で出る
          ——上の「成」ありのケースと**ボタンの位置が一致する**ことを見る。
        */}
        <StudyCase session={applyStudyMoves(KIFU_POSITIONS[0], ['6i7h'])} />
      </Case>

      <Case title="検討・駒を打った直後（「成」が押せる）">
        {/*
          🔴 **打った駒も成らせられる**（決定 2026-08-29）。▲８八角×２二 で得た角を
          ５五へ打った状態。USI に「成って打つ」表記は無いが、検討盤は**合法性を問わない
          フル編集**なので局面編集として筋が通る。押すと５五が成角になり、もう一度で戻る。
          ⚠ **その段は「直前の手の採点」の対象外**になる（`move` を適用した局面と一致しないため）
          ——「評価する」を押しても採点ブロックは出ず、局面評価だけが出る。
        */}
        <StudyCase
          session={tapSquare(
            tapHand(applyStudyMoves(KIFU_POSITIONS[0], ['8h2b']), 'sente', 'B'),
            squareOfUsi('5e'),
          )}
        />
      </Case>

      <Case title="検討中・コントローラー行の評価するが評価中（スピナー）">
        {/*
          🔴 **「評価する」はコントローラー行にある**（決定・2026-09-01）ので、この操作パネルの
          3 つ（棋譜に戻る / 手番 / 成）は押せるままであることを見る。スピナーになるのは
          コントローラー行の 7 つ目だけ。
        */}
        <StudyCase
      ...
      */}
      ...
      {/*
        🔴 **候補手は棋譜側と同じ形に畳む**（prd/12 §3.2・決定 2026-08-29）。
        **md 未満は `PV{n} ▼` の summary だけが出て畳まれ**、md 以上は常時展開になる
        （`app.css` の `details[name='study-candidates']`）。→ **ブラウザ幅を md（768px）の
        上下にまたいで**開閉の違いを見る。⚠ 排他アコーディオンなので 1 本開くと他は閉じ、
        **閉じたときは咎め筋の再生も解除される**（盤が読み専用のまま取り残されないため）。

        ⚠ **評価結果が sticky グループの外に出たことは、ギャラリーでは見えない**
        （ギャラリーの検討盤は sticky を作らない）。実機の棋譜詳細で確認する。
      */}
