...
- **ヘッダー**: 「← 一覧」+ タイトル + ⋯ ドロップダウン（削除ボタンを格納、誤操作防止）
- **将棋盤**: 9x9 テキスト盤面（モバイル `size-8` / md+ `size-10` のレスポンシブ）。先手=黒字、後手=赤字+180度反転。直前の指し手の移動先マスをハイライト表示（`bg-primary/15`）。解析前でも usiMoves があれば盤面表示可能。後手ユーザーの場合はデフォルト反転（盤面反転ボタンはコントローラー行に統合）
- **持ち駒表示**: 盤面の上下に配置（盤面反転で入替）。プレイヤー名を左、持ち駒を右
- **コンパクト情報行**: 直近の指し手 + 局面評価値 + 手数/N + 「分岐」バッジ（分岐中のみ）を 1 行に集約。`flex-wrap` で折り返し
- **コントローラー**: ⏮ ◀ slider ▶ ⏭ + 盤面反転を SVG アイコン化。モバイルでは slider 非表示で ◀▶ を `flex-1` で大型化（タップ操作向け）、md+ では `btn-sm` 化して slider を表示
- **sticky 化**: 盤面 + コンパクト情報行 + コントローラーを `sticky top-0` で画面上端に固定し、スクロール中も常時操作可能
- **局面評価値**: 実手後の局面の先手視点スコア + 形勢判断ラベル（互角/有利/優勢/勝勢/詰み）
- **候補手一覧**: 各候補を `<details name="candidates">` の排他アコーディオンとして展開。展開時は読み筋 + 探索深さ + 実手マーク + 最善手との差異 + 分岐操作 ◀▶ ボタン（`btn-outline btn-sm` + SVG アイコン）を表示。本筋を進めると ref 経由で開いている details を自動クローズ。初期 3 件、4 件以上は chevron で折り畳み。md+ では `cursor-default` で開閉操作を無効化し常時表示ベース
- **悪手判定**: 局面評価値が手番側にとって 300cp 以上悪化し、かつ実手が候補手リスト外の場合に悪手と判定。悪手=赤い▲/△（手番対応）、非悪手の非最善手=※（warning 色）
- **日本語表記**: USI→駒名付き日本語変換（▲７六歩(77)）。盤面追跡で駒名を解決
- **評価値グラフ**: SVG 直書きの折れ線グラフ。先手有利=上、後手有利=下。コントローラー / コンパクト行と連動、クリックで局面移動。悪手マーカー表示（先手▲/後手▽、相手の悪手は透明度を下げる）。現在手位置に縦線を表示。モバイルでは高さ 180px / 余白詰めでナビゲーション用途に強化、デスクトップは 120px
- **LLM 解説用 Markdown エクスポート**: ページ下部の折り畳みセクション「LLM 解説用テキスト」。対局情報・評価値推移表・注目局面（悪手＋評価値変動 top 5、scoreBefore の絶対値 ≥1000cp の局面は除外）・候補手の読み筋を整形した Markdown を生成、クリップボードコピー可。`VITE_SWARS_USER_ID` から先手/後手を判定してユーザー視点プロンプトに切替、注目局面に持ち駒情報も併記、結果コード（`SENTE_WIN_CHECKMATE` 等）を日本語化。実装は `packages/web/src/kifu-export/` に純関数として分離（将来 server / commentator への移植を想定）
- **レイアウト**: モバイルファースト縦並び（sticky グループ → 候補手 → 評価値グラフ → 折り畳み詳細）。md+ も同じ縦並び順序で `max-width` 制御。KIF テキスト・局面評価値テーブル・候補手詳細・LLM 解説用テキスト・メモはページ下部に `<details>` で折り畳み（デフォルト閉じ）
...
### LLM 解説の自動生成 (優先度: 検証中)
- 現状: 棋譜詳細から Markdown を手動コピペ → Claude / ChatGPT で解説生成
- 発展案 1: **commentator 常駐スクリプト**
  - デスクトップ PC（worker 隣）で稼働、解析完了棋譜を検知 → `claude -p` を起動
  - MCP 経由で seseraki から棋譜データを取得、解説を server API 経由で DB 保存
  - Claude Code Max サブスクリプション内で完結、API 課金なし
  - 必要要素: `packages/mcp`（stdio MCP サーバ）、`commentaries` テーブル、`packages/commentator`（cron / loop）
- 発展案 2: **Markdown フォーマット改善**（先行検討中）
  - 戦型ラベル（初手 5〜7 手のパターンマッチ）
  - SFEN を注目局面に併記、評価値推移表に Δ 列
  - 先手/後手の悪手・好手マーカー分離
  - 詳細・LLM フィードバック記録は `docs/kifu-export-feedback.md`（ローカル、`docs/` は gitignore 対象）
- 評価値の精度向上: 開発環境は MATERIAL（駒得ベース）のため序盤評価値が大きめに出る。本番 NNUE（水匠5）で再解析すると安定する想定
