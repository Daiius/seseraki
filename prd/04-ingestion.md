...
| `from` / `to` | `YYYY-MM-DD` | なし | `coalesce(playedAt, createdAt)` に対する期間。**両端を含む**（`to` は翌日 0 時未満）。⚠ **境界は UTC の 0 時**——`from`/`to` は日付文字列のまま DB へ渡り、**DB セッションのタイムゾーン（UTC 固定。[03](./03-data-model.md) §1.1）**で解釈される。**JST の日付では絞れていない**（日本時間 0:00〜9:00 の対局は前日に入る）。🔴 **これは既知の回帰**（レビュー `OCL-85A77255`）——`playedAt` が NULL の棋譜は従来 JST の登録日で絞れていた。JST 境界にするには `periodConditions` 側の変換が要る（境界を `-9h` するだけで済むか、`sourceTz` ごとに変えるかを含めて別途決める）。**未実装・スコープ外** |
...
| POST | `/api/worker/analyses/progress` | 解析の進捗を報告。body `{ kifuId, revision, profile, analyzed, total }`（`profile` は UI の**解析中の表示の濃さ**の出し分けに使う。[05](./05-analysis.md) §1.1b）。**server のメモリに載せるだけで DB を書かない**。`revision` 一致 かつ 失敗記録なし かつ **報告された段階（`profile`）が未完了**のときだけ適用（`{ applied }` を返す）。🔴 **完了は段階ごとに読む**——`analysisCompletedAt` は quick 完了で立つので、**quick 完了後の full 進捗は受理し、full 完了後の報告だけ拒否**する（[05](./05-analysis.md) §1.1b・§1.1d） |
...
> **full の解析を中断して quick を先に処理する**判断を**通信を増やさずに**下せる（[05](./05-analysis.md) §1.1d）。

`POST /api/worker/analyses` の body（`revision` は `GET /api/worker/kifus` で得た取得時の解析世代。
`profile` は同じく取得時に指示された段階。
