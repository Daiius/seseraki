...
> ⚠️ **現状の gap**: 対局メタ抽出・タイトル自動生成は**一括取り込み経路のみ実装済み**で、KIF 貼り付け経路
> （`POST /kifus`）は現在 **`title`（ユーザー入力・必須）+ `kifText` + `usiMoves`（部分的でも保存）のみ**の状態。
> 上記 2 のメタ抽出・result 導出・タイトル自動生成、および §5 の変換ガード（パースエラー/手合割）は**実装で追随する**
> （[08](./08-roadmap.md)）。
