...
- **関係ラベル（`対抗形` / `相居飛車` / `相振り飛車`）は対局単位の性質だが、両手番に複製して保存する**
  （意図的。[01](./01-domain.md) §6.3）。「振り飛車で対抗形のときの自分の勝率」のような**手番別の集計**を
  `(kifuId, side)` の粒度のまま書けるようにするため。手番を持たない行（対局単位の行）を混ぜると、
  同じ表に粒度の違う行が同居して集計のたびに場合分けが要る。
- ⚠ **その代わり `(kifuId, label)` が 2 行になりうるので、素朴な JOIN は二重計上を生む。**
  用途ごとに次の形を使う（実装 PR はこれに従うこと）:

| 用途 | 書き方 |
|---|---|
| 一覧の絞り込み（**どちらかの手番**にそのラベルがある棋譜） | 相関 `EXISTS`（JOIN しない）。JOIN すると `count()` と LIMIT/OFFSET が壊れる（[04](./04-ingestion.md) §6.1 は件数・ページングを同じ条件で数える） |
| 棋譜単位の集計（「角換わりは何局か」） | `COUNT(DISTINCT kifuId)` |
| 手番別の集計（「振り飛車側の勝率」） | `(kifuId, side)` を粒度にする（この粒度なら重複しない） |
