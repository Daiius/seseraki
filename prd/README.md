...
>
> **ただし次は現状に忠実であること**（ここが食い違えば PRD の誤り）: ①各章・[08](./08-roadmap.md) の**「実装済み」記述**は
> 現状を正確に表す、②**既存の DB 列・API** はスキーマに漏らさず載せる、③**設計の意味論**（評価値の視点・moveNumber の定義など）は
> 実データと一致させる。未確定の論点は「要確認」と明記。決定ログは [`_grilling/decisions.md`](./_grilling/decisions.md)。
...
1. **棋譜を溜め込んで自動で解析する** — KIF 貼り付けで棋譜を登録し、対局相手・勝敗・対局日時といった
   基本メタを起こしてタイトルを自動生成する。あとは放っておけば解析が回る。
...
- **実装済み**: KIF 貼り付け登録（メタ抽出・タイトル自動生成・変換ガード込み。[04](./04-ingestion.md) §3）/
  KIF→USI 変換 / 解析失敗のキュー除外・再解析（ポイズンピル対策）/
  worker によるエンジン解析（MultiPV・評価値・候補手読み筋）/ Web による悪手判定・棋譜詳細 UI（盤面・
  評価値グラフ・候補手・日本語表記）/ 戦型ラベルの判定・保存・表示・絞り込み（[01](./01-domain.md) §6）/
  分析ページ（戦型別成績・取りこぼし・一覧へのドリルダウン。[09](./09-analytics.md)）/
...
- フルスタック TypeScript の pnpm monorepo: 現在は `packages/{web,server,worker,shared}`（将来 `commentator`。
  [02](./02-architecture.md) §3 / [08](./08-roadmap.md)）。`shared` は **`board.ts` まで実装済み**で、
  残る抽出（`lib/usi.ts`・悪手判定・kifu-export・zod スキーマ）は gap（[02](./02-architecture.md) §3.2）。
...
```
        [ KIF 貼り付け / CSA 直接貼り付け ]（swars 一括取り込みは無効・§4）
