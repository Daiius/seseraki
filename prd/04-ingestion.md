...
> **対象外**: KIF ファイルアップロード / SFEN（SFEN は単一局面で棋譜解析のスコープ外。[01](./01-domain.md) §7）。

## 3. KIF 貼り付け登録

- Web の棋譜登録画面（`/kifus/new`）でタイトル（任意）+ KIF テキストを貼り付け → `POST /api/kifus`。
  - KIF テキスト欄には**「クリップボードからペースト」ボタン**を置く（`navigator.clipboard.readText`）。
    swars 取り込みの無効化（§4）で **KIF を手貼りするのが主経路**になったため、貼り付けの手数を減らす。
    モバイルはアイコンのみの小さいフットプリント、`sm` 以上でラベルを併記する。クリップボードが空/失敗時は
    既存入力を壊さず `alert` で知らせる。
- server は KIF をパースして次を行う（`packages/server/src/kif`）:
  ...
  2. **対局メタを抽出**（sente/gote/senteDan/goteDan/result/playedAt/sourceTz）。ヘッダ行（`先手：`/`後手：`/`開始日時：`/
     段級表記）から起こす。**段級は 2 通りの書き方を受ける**: 名前欄末尾に空白区切りで付くもの（`先手：羽生善治 九段`）と、
     将棋ウォーズのように**独立した行**に出るもの（`先手段級：二段` / `後手段級：1級`）。両方ある場合は
     **ヘッダの出現順に依らず段級行を優先**し、段級行が段級として解釈できないときだけ名前欄末尾へフォールバックする。
     保持する値は**段=正・級=負**（[01](./01-domain.md) §3 の永続化契約）。**result は終局マーカー（`詰み`/`投了`/`切れ負け`/`千日手`/`持将棋` 等）＋ 手番 parity から
     swars 互換コード（`{SENTE_WIN|GOTE_WIN|DRAW}_{理由}`）を導出**し、既存の勝敗バッジ・LLM エクスポートと揃える
     （[03](./03-data-model.md) / [01](./01-domain.md) §7）。`中断` 等の勝敗なしは null。
     ...
     **`auto` は JST**。かつては UTC で書き出すアプリ（App B）の署名＝「先頭行が柿木形式コメント ＋ `持ち時間：`
     ＋ `終了日時`/`場所` を持たない」に一致したものを UTC と推定していたが、**App B がアップデートで開始日時を JST で
     書くようになり、署名は変わらなかったため廃止した**（残すと新しい KIF を UTC と誤読して 9h 未来へずれる）。
     KIF 側に新旧を分ける手掛かりが無い以上、署名は「一意な肯定指紋を作りにくいため**あくまで初期値の提案**で、
     確定はユーザーの選択に委ねる」という当初の姿勢どおり、**UTC の指定はユーザーに委ねる**。
     決定した TZ を `sourceTz` に残し、`playedAt` は解釈 TZ の絶対時刻で保存する。
...
> ✅ **実装済み**: KIF 貼り付け経路（`POST /api/kifus`）も上記 2 のメタ抽出（sente/gote/段級/日時 + `result` 導出）・
...
| Method | Path | 説明 |
|---|---|---|
| GET | `/api/worker/kifus` | 未解析かつ失敗なし（`analysisCompletedAt IS NULL AND analysisError IS NULL`）の最古を 1 件取得（なければ null）。`usiMoves`・`analysisRevision`・**`analyzedCount`（既に入っている局面数＝解析の再開位置）**を含む |
| POST | `/api/worker/analyses` | 解析結果の**チャンク**をトランザクションで登録（`moveNumber` 単位の追記 upsert。同一 `moveNumber` の再送は入れ直し）。`revision` が現在の `analysisRevision` と一致し、かつ**失敗記録なし・未完了**のときだけ適用し、件数が `usiMoves.length + 1` に達したら同トランザクションで `analysisCompletedAt` を立てる（`{ applied, completed }` を返す。[03](./03-data-model.md) §3 / [05](./05-analysis.md) §1.1c）。`moveNumber` が `0..usiMoves.length` を外れるチャンクは 1 件も書かずに **400** |
| POST | `/api/worker/kifus/:id/error` | 解析失敗を報告し `analysisError` を記録。`revision` 一致時のみ適用（ポイズンピル対策・世代照合。[05](./05-analysis.md) §1.1a） |
| POST | `/api/worker/analyses/progress` | 解析の進捗を報告。body `{ kifuId, revision, analyzed, total }`。**server のメモリに載せるだけで DB を書かない**。`revision` 一致 かつ 未完了・失敗記録なしのときだけ適用（`{ applied }` を返す。[05](./05-analysis.md) §1.1b） |

`POST /api/worker/analyses` の body（`revision` は `GET /api/worker/kifus` で得た取得時の解析世代。
`analyses` は 1 チャンク分の局面で、**空配列もありうる**——解析を終えた時点で未送信の局面が無ければ、
完了を確定させるために空のチャンクを送る。[05](./05-analysis.md) §1.1c）:
