...
| POST | `/kifus/:id/reanalyze` | `kifText` を再パースし `usiMoves`＋メタ列を再生成、`analysisError`/`analysisCompletedAt` をクリアして再キュー（`title`/`memo` は温存）。パーサ修正後の既存棋譜の復旧と失敗棋譜の再試行を兼ねる（[05](./05-analysis.md) §1.1a） |
