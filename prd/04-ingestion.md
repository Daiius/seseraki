...
| `from` / `to` | `YYYY-MM-DD` | なし | `coalesce(playedAt, createdAt)` に対する期間。**両端を含む**（`to` は翌日 0 時未満）。⚠ **境界は UTC の 0 時**——`from`/`to` は日付文字列のまま DB へ渡り、**DB セッションのタイムゾーン（UTC 固定。[03](./03-data-model.md) §1.1）**で解釈される。**JST の日付では絞れていない**（日本時間 0:00〜9:00 の対局は前日に入る）。JST 境界にする案は**未確認・未実装**（境界側を `+9h` する必要があり、`sourceTz` ごとに変える設計まで含めて別途決める） |
