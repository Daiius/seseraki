...
- Web の棋譜登録画面（`/kifus/new`）でタイトル（任意）+ KIF テキストを貼り付け → `POST /kifus`。
- server は KIF をパースして次を行う（`packages/server/src/kif`）:
  ...
  2. **対局メタを抽出**（sente/gote/senteDan/goteDan/result/playedAt/sourceTz）。ヘッダ行（`先手：`/`後手：`/`開始日時：`/
     ...
     **KIF の署名で TZ を判定**する（既定 JST。UTC で書き出すアプリ＝「柿木形式コメント行 ＋ `持ち時間：`」の指紋に
     一致したものだけ UTC 扱い）。判定結果を `sourceTz` に残し、`playedAt` は解釈 TZ の絶対時刻で保存する。
     将来 UTC の別アプリが増えたら署名を追加する（未知アプリは既定 JST ＝安全側）。swars 経路は `gameKey` 由来で常に JST。
...
> 登録後のタイトル編集は現状未実装（`PATCH /kifus/:id` は `memo` のみ。理想では編集可。gap。[08](./08-roadmap.md)）。

> ✅ **実装済み**: KIF 貼り付け経路（`POST /kifus`）も上記 2 のメタ抽出（sente/gote/段位/日時 + `result` 導出）・
> タイトル自動生成、および §5 の変換ガード（パースエラー/非平手手合割 → `usiMoves = null`）を備える。
> 残る gap は登録後のタイトル編集（`PATCH /kifus/:id` は `memo` のみ）と KIF 貼り付けの重複検知（§8）。
...
- **整合チェック（best-effort）**: KIF→USI 変換の結果を投入時に検証する。当面は **パースエラー（指し手行の変換失敗）
  ...
  `GET /worker/kifus` の `usiMoves IS NOT NULL` 条件で自動的に除外される）。**壊れた部分的な `usiMoves` を worker に渡さない**のが
...
| Method | Path | 説明 |
|---|---|---|
| GET | `/kifus` | 棋譜一覧（`coalesce(playedAt, createdAt)` 降順・ページネーション。`hasMemo` を返す） |
| GET | `/kifus/:id` | 棋譜詳細 + 解析結果（moveAnalyses + candidateMoves） |
| POST | `/kifus` | KIF 貼り付け登録。body `{ title?, kifText }` → `{ id }`（title 省略時は自動生成。§3） |
| PATCH | `/kifus/:id` | メモ更新。body `{ memo }`（現状 `memo` のみ。タイトル編集は gap。§3） |
| POST | `/kifus/:id/reanalyze` | `kifText` を再パースし `usiMoves`＋メタ列を再生成、**旧 `moveAnalyses` を削除**し `analysisError`/`analysisCompletedAt` をクリア、**`analysisRevision` を +1** して再キュー（`title`/`memo` は温存）。トランザクション実行。パーサ修正後の既存棋譜の復旧と失敗棋譜の再試行を兼ねる（[05](./05-analysis.md) §1.1a / [03](./03-data-model.md)） |
| DELETE | `/kifus/:id` | 棋譜削除（解析結果も CASCADE） |
| POST | `/swars/import` | swars 取り込みジョブ起動（202・非同期。§4） |
| GET | `/swars/import/status` | swars 取り込みジョブ状態（§4） |
...
| Method | Path | 説明 |
|---|---|---|
| GET | `/worker/kifus` | 未解析かつ失敗なし（`analysisCompletedAt IS NULL AND analysisError IS NULL`）の最古を 1 件取得（なければ null）。`usiMoves` と `analysisRevision` を含む |
| POST | `/worker/analyses` | 解析結果をトランザクションで登録（既存データは DELETE → 再投入で冪等）。`revision` が現在の `analysisRevision` と一致するときだけ適用（`{ applied }` を返す。[03](./03-data-model.md)） |
| POST | `/worker/kifus/:id/error` | 解析失敗を報告し `analysisError` を記録。`revision` 一致時のみ適用（ポイズンピル対策・世代照合。[05](./05-analysis.md) §1.1a） |

`POST /worker/analyses` の body（`revision` は `GET /worker/kifus` で得た取得時の解析世代）:
