...
| Method | Path | 説明 |
|---|---|---|
| GET | `/api/kifus` | 棋譜一覧（`coalesce(playedAt, createdAt)` 降順・ページネーション。`hasMemo` を返す） |
...
| POST | `/api/worker/api/kifus/:id/error` | 解析失敗を報告し `analysisError` を記録。`revision` 一致時のみ適用（ポイズンピル対策・世代照合。[05](./05-analysis.md) §1.1a） |
