...
- **実装済み**: 履歴からの一括取り込み / KIF 貼り付け登録 / KIF→USI 変換と対局メタ抽出 /
...
- フルスタック TypeScript の pnpm monorepo: `packages/{web,server,worker,shared}`（+ 将来 `commentator`）。
...
```
        [ 履歴からの一括取り込み / KIF 貼り付け / (将来)KIF ファイル ]
