...
- 段取り: ✅ (1) `lib/cpl.ts` を `shared` へ移す（[02](./02-architecture.md) §3 の gap を消化）→
  ...
  ✅ (3) `/drills` ページ（出題・採点）→ (4) 問題の一覧と解答履歴のタブ（[13](./13-drills.md) §7.2 / §7.3）→
  (5) 本番で一括生成を一度流す（[13](./13-drills.md) §8）。
...
- ~~✅ **解析来歴（engine/eval）は持たない**~~ → **2026-09-05 に改定**。解析を **`quick` / `full` の 2 段階**に分け、
  **来歴（段階・エンジン名・解析設定）を `moveAnalyses` に記録する**。ただし**上書き・再開の自動判定には使わない**
  （やねうら王の再ビルドで版文字列が変わり、全再解析を招くため）。保存は局面ごとに 1 行のままで、
  full が quick を上書きする（[03](./03-data-model.md) §3 / [05](./05-analysis.md) §1.1d）。
- ✅ **再解析は最新 1 世代に上書き**（[03](./03-data-model.md)）。
...
- **解析エンジンの二重性**: 開発 MATERIAL と本番 NNUE で評価値が変わる。**来歴は記録するが自動判定には使わず**、
  本番 NNUE 単一運用で受ける（改定・2026-09-05。[03](./03-data-model.md) §3 / [05](./05-analysis.md) §1.1d）。
  異常値は目視で気づける前提。同じ棋譜でもエンジンで
