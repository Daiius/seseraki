...
pnpm db:backfill-tz  # 既存棋譜の playedAt を KIF 署名の TZ 判定で再導出し sourceTz を埋める（db:migrate の後に一度）
...
> 失敗する）。backfill は `kifText` から `playedAt`/`sourceTz` を再導出する冪等処理。dev DB に試すときは `db:backfill-tz:dev`。
