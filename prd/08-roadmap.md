...
### 戦型ラベル（優先度: 高・設計確定済み / 実装は未着手）

- 判定方式は**確定済み**（内部 / 一次 / 二次の 3 層 + 「駒がぶつかるまで」の観測窓。[01](./01-domain.md) §6）。
  スキーマも確定（`kifuTactics`。[03](./03-data-model.md) §2.1）。**残るのは実装**。
- 段取り: (1) 判定ロジックを `shared` へ純関数として置く → (2) 投入時に判定して `kifuTactics` へ保存 +
  一括再判定の口 → (3) 一覧のフィルタ軸・「同じ戦型の過去局へ」の導線 → (4) LLM プロンプトへ添える
...
- ✅ **`packages/shared` を新設し、盤面追跡（`board.ts`）を web から抽出済み**（テスト 19 件ごと移設）。
  戦型判定を server から回すのに必要だったため先行して消化した。
- usi 変換・USI→日本語表記・悪手判定・kifu-export・zod 検証スキーマの抽出は**まだ gap**（web にある）。
