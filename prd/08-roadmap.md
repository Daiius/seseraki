...
- **投入**: 履歴からの一括取り込み（手動トリガー・非同期ジョブ）/ KIF 貼り付け登録（指し手のみ。[04](./04-ingestion.md)）。
- **変換・メタ抽出**: CSA→KIF→USI 変換、対局者・勝敗・日時の抽出、対局タイトル生成（**一括取り込み経路のみ**。[01](./01-domain.md) / [04](./04-ingestion.md)）。
- **解析**: worker がやねうら王 + 評価関数で MultiPV 解析、評価値・悪手判定・候補手読み筋を登録（[05](./05-analysis.md)）。
- **可視化**: 棋譜詳細（盤面・評価値グラフ・候補手・日本語表記・悪手マーカー）、棋譜一覧（[05](./05-analysis.md)）。
...
- 1 時間に 1 回程度の自動取り込み（現状は手動トリガーのみ）。
- 頻度は控えめに保つ（運用上の詳細は `.claude-personal/` の運用メモ）。
