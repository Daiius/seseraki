...
| POST | `/api/worker/analyses` | 解析結果の**チャンク**をトランザクションで登録（`moveNumber` 単位の追記 upsert。同一 `moveNumber` の再送は入れ直し）。`revision` が現在の `analysisRevision` と一致するときだけ適用し、件数が `usiMoves.length + 1` に達したら同トランザクションで `analysisCompletedAt` を立てる（`{ applied, completed }` を返す。[03](./03-data-model.md) §3 / [05](./05-analysis.md) §1.1c） |
